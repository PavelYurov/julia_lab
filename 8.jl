...
mutable struct Polinom
    coef::Array
    Polinom(a::Array) = new(a)
    Polinom(a) = new([a])
end

struct Residu{T,M}
    a::T
    Residu{T,M}(b::T)  where{T, M} = new(mod(b,M))
end

function gcp(a::T, b::T) where T
    while(b!=0)
        a,b = b,mod(a,b)
    end
    return a
...
function gcp(a::Polinom, b::NTuple)
    c = Polinom(convert_to_array(b))
    while(c != Polinom([0]))
        a,c = c,mod(a,c)
    end
    return a
...
function egcp(a::T, b::T) where T <: Int
    ...
    u , v = 1 , 0;
    u1,v1=0,1
    while(b>0) #ИНВАРИАНТ: НОД(a,b) = HОД(a0,b0) && a = u*a0 + v*b0 && b = u_*a0 + v_ * b0
        k = div(a,b)
        a,b = b,a-k*b
...
function egcp(a::Polinom, z::NTuple)
    b=Polinom(convert_to_array(z))
    m,n = a,b
    u , v = Polinom(1) , Polinom(0);
    u1,v1=Polinom(0),Polinom(1)
    while(b!=Polinom(0)) #ИНВАРИАНТ: НОД(a,b) = HОД(a0,b0) && a = u*a0 + v*b0 && b = u_*a0 + v_ * b0
        k = (a/b)[1]
        a,b = b,a-k*b
        u,v,u1,v1 = u1,v1,u-k*u1,v-k*v1
    end
...
function inverse(a::T,n::T) where T
    if(gcp(a,n) != 1)
        return Nothing
    end
...
Base. mod(a::Polinom,b::Polinom) = (a/b)[2]
function convert_to_array(b::NTuple)
    a = []
    for i in b[1:end]
        push!(a,i) 
    end
...
function Base. mod(a::Polinom, b::NTuple)
    c = Polinom(convert_to_array(b))
    return (a/c)[2]
end
...
function Base. *(a::Residu{Polinom,M},b::Polinom) where M
    arr = a.a*b
    return arr
end
...
# function Base. /(a::Residu{Polinom,T},b::Residu{Polinom,T})where T
#     return Residu{Polinom,T}((a.a/b.a)[1])
# end
...
r = 12
a = Residu{Polinom,(-1,0,0,1)}(Polinom([Residu{Int,r}(1),Residu{Int,r}(2),Residu{Int,r}(1)]))
println()
b = Residu{Polinom,(-1,0,0,1)}(Polinom([Residu{Int,r}(1),Residu{Int,r}(1)]))
println()
a/b |>print
