...
#================================================================#
mutable struct Polinom{T}
    coef::Array{T}
end

function Base.print(a::Polinom{T})where T
...
function Base. +(a::Polinom{T}, b::Polinom{T})where T
    b_ = b.coef
    a_ = a.coef
...
function Base. -(a::Polinom{T}, b::Polinom{T}) where T
    c = Polinom{T}(T(-1)*b.coef )                                 #!!!!
    return a + c;
end

function normalize(a::Polinom{T})where T
    while size(a.coef)[1] > 1 && a.coef[end] == T(0) 
        pop!(a.coef)
    end
end

function dim(a::Polinom{T})where T
    normalize(a)
...
function Base. *(a::Polinom{T}, b::Polinom{T})where T
    ...
    return Polinom{T}(arr)
end
function Base. *(value::T, a::Polinom{T}) where T
    c = Polinom{T}(value * a.coef)
    return c
end
...
function Base. *(a::Residu{T,M},b::Polinom{Residu{T,M}}) where {T,M}
    arr = a*b.coef
    return Polinom{Residu{T,M}}(arr)
end
...
function Base. /(a::Polinom{T}, b::Polinom{T})where T
    ...
    while dim(ost) >= dim(b) && ost.coef[end] != T(0)
        # println(Polinom{T}([ost.coef[end]/b.coef[end]]))
        # println(size(a.coef)[1])
        # println(get_max_pow(dim(ost) - dim(b)))
        chel = chel + Polinom{T}([ost.coef[end]/b.coef[end]])*get_max_pow(dim(ost) - dim(b),T)
...
#b2 = Residu{Int,R}(3)
#b3 = Residu{Int,R}(1)
a = Polinom{Residu{Int,R}}([a4,a3,a2,a1])
b = Polinom{Residu{Int,R}}([b1])
